...
import { CardReader } from 'pcsclite';
...
class Reader extends EventEmitter {
  ...
  protected connection: Nullable<Connection> = null;
  protected card: Nullable<Card> = null;

  protected _aid: Nullable<Buffer | Function> = null;

  protected keyStorage: Record<string, Nullable<Buffer | Uint8Array>> = {
    0: null,
    1: null,
  };

  protected pendingLoadAuthenticationKey: Partial<
  ...
  static selectStandardByAtr(atr: Buffer): Standard {
  ...
  get aid(): Nullable<Buffer | Function> {
    return this._aid;
  }

  set aid(value: Buffer | string) {
  ...
  constructor(reader: CardReader, logger: Logger) {
    super();
      ...
      if (changes) {
        if (
          ...
          try {
            ...
            this.emit('error', exception as Error);
          }
        } else if (
  ...
  connect(mode: Mode = CONNECT_MODE_CARD) {
  ...
  transmit(data: Buffer, responseMaxLength: number): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Connection is defined due to the guard throwing TransmitError
  ...
  async loadAuthenticationKey(
    ...
    const packet = Buffer.from([
      ...
      buffer.length, // Lc: Length of the key (6)
      ...buffer, // Data In: Key (6 bytes)
    ]);

    let response: Nullable<Buffer> = null;
    ...
    try {
      ...
      throw new LoadAuthenticationKeyError(null, null, exception as Error);
    }

    this.keyStorage[keyNumber] = buffer;
  ...
  async authenticate(
    ...
    if (keyNumber === undefined) {
      ...
      if (!this.pendingLoadAuthenticationKey[key.toString()]) {
        ...
        if (this.keyStorage[keyNumber] !== null) {
          ...
          const freeNumber = Object.keys(this.keyStorage).find(
            (item) => this.keyStorage[item] === null,
          );
        ...
        this.pendingLoadAuthenticationKey[key.toString()] =
          this.loadAuthenticationKey(parseInt(keyNumber), key);
      }

      try {
        const value = await this.pendingLoadAuthenticationKey[key.toString()];
        keyNumber = value?.toString();
      } catch (exception) {
        ...
        delete this.pendingLoadAuthenticationKey[key.toString()];
      }
    }

    authKey = parseInt(keyNumber!);
    ...
    try {
      ...
      throw new AuthenticationError(null, null, exception as Error);
    }
  ...
  async read(
    ...
    if (length > packetSize) {
      const packet = Math.ceil(length / packetSize);
      ...
      for (let i = 0; i < packet; i++) {
        ...
        const size =
          (i + 1) * packetSize < length ? packetSize : length - i * packetSize;
    ...
    try {
      ...
      throw new ReadError(null, null, exception as Error);
    }
  ...
  async write(
    ...
    if (data.length > blockSize) {
      const packet = data.length / blockSize;
      ...
      return Promise.all(commands).then((values) => values.every(Boolean));
    }
    ...
    try {
      ...
      throw new WriteError(null, null, exception as Error);
    }
  ...
  async handle_Iso_14443_3_Tag() {
    ...
    try {
      ...
      const error = new GetUIDError(null, null, exception as Error);
