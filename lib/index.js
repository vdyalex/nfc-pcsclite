...
module.exports = function () {
  ...
  console.log('instantiating PCSCLite', PCSCLite);
  const pcsc = new PCSCLite();
  console.log('PCSCLite instantiated', pcsc);

  console.log('defining readers', readers);
  pcsc.readers = readers;
  console.log('readers defined', pcsc.readers);

  process.nextTick(function () {
    console.log('setting nextTick');
    pcsc.start(function (error, data) {
      console.log('pcsc.start', error, data);

      if (error) {
        console.log('pcsc.start error', error);
        return pcsc.emit('error', error);
      }

      console.log('setting pcsc.start names', data);
      const names = parseReadersString(data);
      console.log('pcsc.start names set', names);
